...
async function updateMatchInfos(matchId: number) {
    ...
    if(hltvInfos.id) { // Is the match valid ?
        ...
        if (hltvInfos.maps.length > 0) { await makeTwitchJSONfile(matchId, twitchStreams, hltvInfos.maps.length) }
    }

    return hltvInfos.available
}

async function findMatchInfos(matchId: number, mapNumber: number): Promise<MatchInfos> {
